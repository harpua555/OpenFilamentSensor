...
> [!WARNING]
> This distributor is no longer hosted. The code is preserved here for historical reference or local use.
...
- On load, it fetches the latest GitHub Releases list for `YOUR_GITHUB_USERNAME/YOUR_GITHUB_REPO` and populates a release selector (or a specific tag via `?tag=vX.Y.Z`).
